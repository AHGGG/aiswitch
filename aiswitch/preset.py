...
from .config import ConfigManager, PresetConfig, ProjectConfig
...
class PresetManager:
    ...
    def add_preset(
        ...
        variables = {"API_KEY": api_key, "API_BASE_URL": base_url}
    ...
    def use_preset(
        self, name: str, apply_to_env: bool = True
    ) -> Tuple[PresetConfig, Dict[str, str], List[str]]:
        preset = self.config_manager.get_preset(name)
        if not preset:
            similar = self.config_manager.get_similar_preset_names(name)
            if similar:
                raise ValueError(
                    f"Preset '{name}' not found. Did you mean: {', '.join(similar)}?"
                )
            else:
                raise ValueError(
        ...
        if apply_to_env:
            ...
            applied_vars, cleared_vars = self.env_manager.apply_preset(
                preset, current_preset=current_preset
            )
    ...
    def save_project_config(
        ...
        if preset_name is None:
            ...
            if not current:
                raise ValueError(
                    "No current preset to save. Use 'aiswitch use <preset>' first."
                )
        ...
        project_config = ProjectConfig(preset=preset_name, overrides=overrides or {})

        self.config_manager.save_project_config(project_config, project_dir)
        return project_config

    def load_project_config(
        self, project_dir: Optional[Path] = None, apply_to_env: bool = True
    ) -> Tuple[PresetConfig, Dict[str, str], List[str]]:
        ...
        if not preset:
            raise ValueError(
                f"Preset '{project_config.preset}' referenced in project config not found"
            )
        ...
        if project_config.overrides:
            try:
                validated_overrides = self.env_manager.validate_env_variables(
                    project_config.overrides
                )
        ...
        if apply_to_env:
            ...
            current_preset = self.get_current_preset()
            applied_vars, cleared_vars = self.env_manager.apply_preset(
                merged_preset, current_preset=current_preset
            )
    ...
    def export_all_presets(
        self, output_file: Optional[Path] = None, redact_secrets: bool = True
    ) -> Dict:
        """导出所有预设"""
        preset_names = self.config_manager.list_presets()
        if not preset_names:
    ...
    def export_preset_to_file(
        self, name: str, output_file: Path, redact_secrets: bool = True
    ):
        """导出单个预设到文件"""
    ...
    def import_preset(
        self, preset_data: Dict, allow_overwrite: bool = False
    ) -> PresetConfig:
        """导入单个预设"""
        try:
        ...
        if self.config_manager.preset_exists(preset.name) and not allow_overwrite:
            raise ValueError(
                f"Preset '{preset.name}' already exists. Use --force to overwrite."
            )
        ...
        for key, value in preset.variables.items():
            if value == "***REDACTED***":
                raise ValueError(
                    f"Cannot import preset with redacted value for '{key}'. Please update the value before importing."
                )
    ...
    def import_from_file(
        self, input_file: Path, allow_overwrite: bool = False
    ) -> List[PresetConfig]:
        """从文件导入预设"""
        ...
        if "preset" in data:
            ...
            for preset_data in data["presets"]:
                try:
                    ...
                    print(
                        f"Warning: Failed to import preset '{preset_data.get('name', 'unknown')}': {e}"
                    )
        else:
            raise ValueError(
                "Invalid import file format. Expected 'preset' or 'presets' key."
            )
    ...
    def update_preset(self, name: str, **kwargs) -> PresetConfig:
        ...
        try:
            validated_vars = self.env_manager.validate_env_variables(
